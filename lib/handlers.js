...
const handle_body = (nodes, state) => {
  ...
  for (const statement of nodes) {
    ...
    while (state.comments.length) {
      ...
      state.commands.push(add_newline ? newline : ref`#`, WS` `, { type: 'Comment', comment });
...
const handle_var_declaration = (node, state) => {
  ...
  state.commands.push(KW(node.kind), ref`#`, WS` `, open);
...
function sequence(nodes, state, spaces, fn, separator = [ref`children[]`, PN`,`]) {
...
function handle_type_annotation(node, state) {
  switch (node.type) {
    ...
    case 'TSFunctionType':
      ...
      state.commands.push(ref`children[]`, PN`)`, ref`#`, WS` `, ref`children[]`, PN`=>`, WS``);
...
const handlers = {
  ...
  ArrowFunctionExpression: (node, state) => {
    ...
    state.commands.push(ref`children[]`, PN`(`);
    sequence(node.params, state, false, handle);
    state.commands.push(ref`children[]`, PN`)`, ref`#`, WS` `, ref`children[]`, PN`=>`, ` `);
  ...
  AssignmentExpression(node, state) {
    handle(node.left, state);
    state.commands.push(ref`#`, WS` `, PN(node.operator), ref`#`, WS` `);
    handle(node.right, state);
  },
  ...
  ForStatement: (node, state) => {
    ...
    state.commands.push(ref`statementTerminatorToken`, PN`;`, ref`#`, WS` `);
  ...
  Literal(node, state) {
    ...
    state.commands.push(...map(buildAppend, [...streamFromTree(cstNode)].slice(1, -1)));
  ...
  MethodDefinition(node, state) {
    ...
    if (node.kind === 'get' || node.kind === 'set') {
      // Getter or setter
      state.commands.push(KW(node.kind), ref`#`, WS` `);
    }
    ...
    sequence(node.properties, state, true, (p, state) => {
      if (p.type === 'Property' && p.value.type === 'FunctionExpression') {
        ...
        if (p.kind === 'get' || p.kind === 'set') {
          state.commands.push(KW(p.kind) + ref`#`, WS` `);
        } else {
  ...
  PropertyDefinition(node, state) {
    if (node.accessibility) {
      state.commands.push(KW(node.accessibility), ref`#`, WS` `);
    }
  ...
  SwitchStatement(node, state) {
    ...
    for (const block of node.cases) {
      if (!first) state.commands.push(WS`\n`);
      first = false;
  ...
  TemplateLiteral(node, state) {
    ...
    for (let i = 0; i < expressions.length; i++) {
      ...
      state.commands.push(LIT(raw), PN('${'));
  ...
  UnaryExpression(node, state) {
    state.commands.push(PN(node.operator));
